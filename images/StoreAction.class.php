...
// 		}
	 ...
	 * 返回所有自身+子目录Id
	 */
	public function getAllCatsId($cats)
	{
		$ret = array();
		$ret[] = $cats['id'];

		foreach($cats['sub'] as $k => $v){
				$ret = array_merge($ret, $this->getAllCatsId($v));
		}
	 ...
	 * 返回所有自身+兄弟+子目录Id
	 */
	public function getParentCatsId($cats)
	{
		$ret = array();
		foreach($cats as $k => $v){
			$ret = array_merge($ret, $this->getAllCatsId($v));
	...
	{
		if (IS_POST && !empty($_POST['search_name']) ){
			$key = $this->_post('search_name');
			$this->redirect('/index.php?g=Wap&m=Store&a=products&token=' . $this->token . '&wecha_id=' . $this->wecha_id . '&keyword=' . $_POST['search_name'] . '&twid=' . $this->_twid);
		}
		...
		foreach($result as $k => $v){
			...
			$cts = $this->getAllCatsId($v);

			if(count($cts) > 1){
				$where['catid'] = array('in',$cts);
			}else {
				$where['catid'] = $cts[0];
			}

			$bst = $this->product_model->where($where)->order('salecount desc')->limit('0, 6')->select();
			$result[$k]['best'] = $bst;
		}
		$this->assign('info', $result);

		//get all categories to query new products and best seller in each categories

		$allcats = $this->getParentCatsId($result);
		$where = array('token' => $this->token, 'cid' => $this->_cid, 'groupon' => 0, 'dining' => 0, 'status' => 0);
		$where['catid'] = array('in', $allcats);

		$bestseller = $this->product_model->where($where)->order('salecount desc')->limit('0, 8')->select();
